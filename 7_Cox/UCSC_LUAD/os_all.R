...
data <- merge(UCSC_LUAD_exprs_RS_all,LUAD1,by='Sample',all = F)
...
write.table(data,"data_all.txt",sep="\t",row.names=F,quote=F, na="")
...
write.table(UCoxR,"UCoxR_os_all.txt",sep="\t",row.names=F,quote=F)
...
write.table(MCoxR,"MCoxR_os_all.txt",sep="\t",row.names=F,quote=F)


Final<- merge.data.frame(UCoxR,MCoxR,by='Factor',all=F,sort=F)
write.table(Final,"Final_os_all.txt",sep="\t",row.names=F,quote=F,na='')


rm(list=ls())
path <- getwd()
setwd(path)
UCoxR_os<-read.table("UCoxR_os_all.txt",sep="\t",header=T,check.names=F)  
...
pdf("Uni_os_all.pdf", onefile = FALSE)
...
path <- getwd()
setwd(path)
MCoxR_os<-read.table("MCoxR_os_all.txt",sep="\t",header=T,check.names=F)  
...
write.table(Mul_os,"Mul_os_all.txt",sep="\t",row.names=F,quote=F)


library('forestplot')
test<-read.table("Mul_os_all.txt",sep="\t",header=F,check.names=F)
pdf("Mul_os_all.pdf", onefile = FALSE,width = 12,height = 7)
forestplot(labeltext = as.matrix(test[,1:3]),  
           hrzl_lines =list("2"= gpar(lwd=1, columns=1:3, col = "#000044")),
           mean = test$V4, 
           lower = test$V5, 
