...
GSE41271_exprs_RS_test <- read.table("GSE41271_exprs_RS_test.txt",sep="\t",header=T,check.names=F) 
LUAD <- read.table("LUAD.txt",sep="\t",header=T,check.names=F)
LUAD1  <- LUAD[,c('Sample','Age','Gender','Stage','Smoking')]

data <- merge(GSE41271_exprs_RS_test,LUAD1,by='Sample',all = F)
...
write.table(data,"data_GSE41271.txt",sep="\t",row.names=F,quote=F, na="")
...
write.table(UCoxR,"UCoxR_os_GSE41271.txt",sep="\t",row.names=F,quote=F)
...
write.table(MCoxR,"MCoxR_os_GSE41271.txt",sep="\t",row.names=F,quote=F)


Final<- merge.data.frame(UCoxR,MCoxR,by='Factor',all=F,sort=F)
write.table(Final,"Final_os_GSE41271.txt",sep="\t",row.names=F,quote=F,na='')


rm(list=ls())
path <- getwd()
setwd(path) 
UCoxR_os<-read.table("UCoxR_os_GSE41271.txt",sep="\t",header=T,check.names=F)  
...
write.table(Uni_os,"Uni_os_GSE41271.txt",sep="\t",row.names=F,quote=F)


library('forestplot')
test<-read.table("Uni_os_GSE41271.txt",sep="\t",header=F,check.names=F)
pdf("Uni_os_GSE41271.pdf")
...
path <- getwd()
setwd(path)  
MCoxR_os<-read.table("MCoxR_os_GSE41271.txt",sep="\t",header=T,check.names=F)  
...
write.table(Mul_os,"Mul_os_GSE41271.txt",sep="\t",row.names=F,quote=F)


library('forestplot')
test<-read.table("Mul_os_GSE41271.txt",sep="\t",header=F,check.names=F)
pdf("Mul_os_GSE41271.pdf")
forestplot(labeltext = as.matrix(test[,1:3]),  
           hrzl_lines =list("2"= gpar(lwd=1, columns=1:3, col = "#000044")),
           mean = test$V4, 
           lower = test$V5, 
