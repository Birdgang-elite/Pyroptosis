...
GSE50081_exprs_RS_test <- read.table("GSE50081_exprs_RS_test.txt",sep="\t",header=T,check.names=F) 
LUAD <- read.table("LUAD.txt",sep="\t",header=T,check.names=F)
LUAD1  <- LUAD[,c('Sample','Age','Gender','pT','pN','Stage','Smoking')]

data <- merge(GSE50081_exprs_RS_test,LUAD1,by='Sample',all = F)
...
data$pT <- factor(data$pT,levels=c(1,2,3),labels=c('T1','T2-3','T2-3'))
data$pN <- factor(data$pN,levels=c(0,1),labels=c('N0','N1'))
data$Stage <- factor(data$Stage,levels=c(1,2),labels=c('I','II'))
data$Smoking <- factor(data$Smoking,levels=c(0,1),labels=c('Nonsmoker','Smoker'))
write.table(data,"data_GSE50081.txt",sep="\t",row.names=F,quote=F, na="")
...
for(a in c('Age','Gender','pT','pN','Stage','Smoking','Risk_score')){  
  unicox=coxf(a)
  UCox = summary(unicox)
...
write.table(UCoxR,"UCoxR_os_GSE50081.txt",sep="\t",row.names=F,quote=F)


UCoxR$`P Value`=as.numeric(as.character(UCoxR$`P Value`)) 
paste0(UCoxR$Factor,collapse = '+')  
mulcox <- coxph(Surv(os_time,os_status) ~ Age+Gender+pT+pN+Stage+Smoking+Risk_score,
                data=cli_exp)  
MCox=summary(mulcox)
...
write.table(MCoxR,"MCoxR_os_GSE50081.txt",sep="\t",row.names=F,quote=F)


Final<- merge.data.frame(UCoxR,MCoxR,by='Factor',all=F,sort=F)
write.table(Final,"Final_os_GSE50081.txt",sep="\t",row.names=F,quote=F,na='')


rm(list=ls())
path <- getwd()
setwd(path)
UCoxR_os<-read.table("UCoxR_os_GSE50081.txt",sep="\t",header=T,check.names=F)  
...
write.table(Uni_os,"Uni_os_GSE50081.txt",sep="\t",row.names=F,quote=F)


library('forestplot')
test<-read.table("Uni_os_GSE50081.txt",sep="\t",header=F,check.names=F)
pdf("Uni_os_GSE50081.pdf", onefile = FALSE)
...
path <- getwd()
setwd(path)
MCoxR_os<-read.table("MCoxR_os_GSE50081.txt",sep="\t",header=T,check.names=F)  
...
write.table(Mul_os,"Mul_os_GSE50081.txt",sep="\t",row.names=F,quote=F)


library('forestplot')
test<-read.table("Mul_os_GSE50081.txt",sep="\t",header=F,check.names=F)
pdf("Mul_os_GSE50081.pdf", onefile = FALSE,width = 15,height = 7)
forestplot(labeltext = as.matrix(test[,1:3]),  
           hrzl_lines =list("2"= gpar(lwd=1, columns=1:3, col = "#000044")),
           mean = test$V4, 
           lower = test$V5, 
