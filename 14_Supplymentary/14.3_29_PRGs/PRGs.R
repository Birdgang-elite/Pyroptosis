...
ggsave('chord_BP.pdf',plot = p1,width = 15,height = 15) 

p2 <- GOChord(data = chord_CC,title = 'GO-Cellular Component',space = 0.01,
              limit = c(1,1),gene.order = 'logFC',gene.space = 0.25,gene.size = 5,
              lfc.col = c('red','white','blue'), 
              process.label = 10) 
ggsave('chord_CC.pdf',plot = p2,width = 15,height = 15) 

p3 <- GOChord(data = chord_MF,title = 'GO-Mollecular Function',space = 0.01,
              limit = c(1,1),gene.order = 'logFC',gene.space = 0.25,gene.size = 5,
              lfc.col = c('red','white','blue'), 
              process.label = 10)
ggsave('chord_MF.pdf',plot = p3,width = 15,height = 15) 


#KEGG富集弦图
KEGGplotIn<-ekk[1:5,c(1,2,6,8)] #提取KEGG的前10行,提取ID,Description,p.adjust,GeneID四列
KEGGplotIn$geneID <-str_replace_all(KEGGplotIn$geneID,'/',',') #把GeneID列中的’/’替换成‘,’
names(KEGGplotIn)<-c('ID','Term','adj_pval','Genes')#修改列名,后面弦图绘制的时候需要这样的格式
KEGGplotIn$Category = "KEGG"#分类信息
circ_KEGG<-GOplot::circle_dat(KEGGplotIn,genedata) #GOplot导入数据格式整理
chord_KEGG<-chord_dat(data = circ_KEGG,genes = genedata) #生成含有选定基因的数据框
p1 <- GOChord(data = chord_KEGG,#弦图
              title = 'KEGG Process',space = 0.01,#GO Term间距
              limit = c(1,1),gene.order = 'logFC',gene.space = 0.25,gene.size = 5,
              lfc.col = c('red','white','blue'), #上下调基因颜色
              process.label = 10) #GO Term字体大小
ggsave('chord_KEGG.pdf',plot = p1,width = 15,height = 15) 
