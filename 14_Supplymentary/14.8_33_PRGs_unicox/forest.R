...
#读取表达谱和预后数据
load('res.RData')
PRGs <- read.table('33PRGs.txt', sep = '\t', header = T, check.names = F)

train_data <- list_train_vali_Data$TCGA
est_dd <- as.data.frame(train_data)[, c('ID','OS.time','OS', intersect(colnames(train_data),PRGs$gene))]
...
for(a in intersect(colnames(train_data),PRGs$gene)){
  unicox=coxf_all(a)
  UCox = summary(unicox)
...
forestplot(labeltext = as.matrix(test[,1:3]),  
           ...
           lower = test$V5, 
           upper = test$V6, 
           is.summary=c(T,F,F,F,F,F,F,F,F,F,F,F,F,F,F,F,F,F,F,F,F,F,F,F,F,F,F,F,F,F,F,F,F),
